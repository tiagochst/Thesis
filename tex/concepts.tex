...
\begin{description}
  ...
  \item [Copy Number Variation data:] The processed data is a file with detected duplications or deletions of contiguous chromosomal regions by measuring array intensity levels. This files that might contain or not germline CN variations (files with  the prefix "nocnv" had the removed). This processed data is used as input for \sigla{CBS}{circular binary segmentations}  which divides each chromosome into contiguous segments of equal copy-number and quantifies each.
  \item [Histone ChIP-Seq data:] The processed data contains either the fold change over control and signal p-value stored
  in a bigWig format or as peaks in a bed and bigBed format. \cite{ENCODE_chipseq}
\end{description}
