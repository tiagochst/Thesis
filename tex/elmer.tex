...
Here we describe each of following analysis steps shown in Figure \ref{fig:elmerworkflow}: i) Organize data as a \textit{MultiAssayExperiment} object; ii) Identify distal probes with significantly different DNA methylation
   level when comparing two sample groups; iii) Identify putative target genes for differentially methylated
   distal probes, using methylation vs. expression correlation; iv) Identify enriched motifs for each probe belonging to a significant probe-gene pair; v) Identify master regulatory Transcription Factors (TF)
